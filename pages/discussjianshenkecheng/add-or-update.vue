	...
	<view :style='{"minHeight":"100vh","width":"100%","padding":"20rpx 40rpx","position":"relative","background":"#edeff3","height":"auto"}'>
		<form :style='{"width":"100%","padding":"24rpx","background":"#edeff3","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":" 20rpx","margin":"0 0 24rpx 0","borderColor":"#ccc","borderRadius":"20rpx","borderWidth":"0","background":"#fff","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"auto","margin":"0 10rpx","lineHeight":"80rpx","fontSize":"28rpx","color":"#9E9E9E","fontWeight":"500"}' class="title">评论内容</view>
                <xia-editor ref="editor" :style='{"minHeight":"300rpx","border":"0px solid #efefef","padding":"10rpx","flex":"1","background":"#EDEFF3","height":"auto"}' v-model="ruleForm.content" placeholder="评论内容" @editorChange="contentChange"></xia-editor>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussjianshenkecheng`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussjianshenkecheng`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussjianshenkecheng`, this.ruleForm);
						}else{
							await this.$api.add(`discussjianshenkecheng`, this.ruleForm);
						}
						uni.setStorageSync('discussjianshenkechengCleanType',true);
