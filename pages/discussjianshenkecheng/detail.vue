...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussjianshenkechengCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussjianshenkechengCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussjianshenkecheng', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discussjianshenkecheng');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discussjianshenkecheng');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discussjianshenkecheng', this.id);
				this.detail = res.data;

				if(type==2){
					this.detail.discussnum++
					await this.$api.update('discussjianshenkecheng',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussjianshenkecheng', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussjianshenkecheng/add-or-update?refid=${this.id}`)
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussjianshenkecheng',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussjianshenkecheng',that.detail)
							setTimeout(()=>{
